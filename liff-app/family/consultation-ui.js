(function initFamilyConsultationUI(root,factory){
  ...
  function createController({doc,session}){
    ...
    function render(state){
      ...
      el.panel.hidden=!state.visible;el.patient.textContent=state.patientName?`Care Profile ที่เลือก: ${state.patientName}`:'';el.entry.hidden=state.screen!=='overview';el.refresh.hidden=!state.visible;el.flow.hidden=state.screen==='overview';el.question.hidden=state.screen!=='question';el.check.hidden=state.screen!=='question';el.eligible.hidden=state.screen!=='eligible';el.eligibleContinue.disabled=!state.eligibility?.termsVersion;el.cancel.hidden=state.screen==='overview';el.terms.hidden=state.screen!=='terms';el.termsVersion.textContent=state.eligibility?.termsVersion?`เงื่อนไขฉบับ ${state.eligibility.termsVersion}`:'';el.payment.hidden=state.screen!=='payment';el.continueButton.disabled=!state.termsAccepted||!state.eligibility?.termsVersion||state.loading;el.termsCheck.checked=state.termsAccepted;el.live.textContent=state.statusMessage;if(el.paymentText)el.paymentText.textContent=paymentStatusLabel(state.paymentStatus);if(el.paymentQr){const qr=safePaymentQrUrl(state.paymentInstructions?.qrImageUrl);el.paymentQr.hidden=!qr;if(qr)el.paymentQr.src=qr;else el.paymentQr.removeAttribute('src');}renderCaseGroups(doc,el.groups,state.collections,(id)=>session.selectCase(id));
