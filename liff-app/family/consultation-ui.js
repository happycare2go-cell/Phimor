(function initFamilyConsultationUI(root,factory){
  ...
  function createFamilyConsultationSession({request,onChange=()=>{},schedule=setTimeout,cancelSchedule=clearTimeout,pollSeconds=5,cryptoApi=globalThis.crypto,now=()=>new Date(),checkoutAdapter=null,paymentStatusAdapter=null}={}){
    ...
    async function continueToPayment(){
      ...
      try{const result=await checkoutAdapter({careProfileId:state.profileId,question:state.question,termsVersion:state.eligibility.termsVersion});replace({paymentStatus:result?.status||'payment_confirming',paymentOrderId:result?.orderId||null,paymentInstructions:result?.payment||null,statusMessage:result?.status==='payment_pending'?'สแกน PromptPay QR และรอระบบยืนยันการชำระเงิน':''});if(result?.orderId)schedulePaymentPoll(result.orderId);return result;}
    ...
    async function pollPaymentStatus(orderId=state.paymentOrderId){
      ...
      try{const result=typeof paymentStatusAdapter==='function'?await paymentStatusAdapter({orderId}):await request(`/api/consultations/orders/${encodeURIComponent(orderId)}/status`);if(!current(token,profileId)||state.paymentOrderId!==orderId)return {ignored:true,stale:true};replace({paymentStatus:result?.status||'payment_confirming',statusMessage:result?.status==='queued'?'ชำระเงินแล้ว • รอเภสัชกรรับเคส':['failed','expired'].includes(result?.status)?'การชำระเงินไม่สำเร็จหรือหมดอายุ':''});if(result?.status==='queued'){stopPaymentPolling();await refreshCases();}else if(!['failed','expired'].includes(result?.status))schedulePaymentPoll(orderId);return result;}catch(error){if(current(token,profileId))replace({statusMessage:'กำลังรอการยืนยันการชำระเงิน'});schedulePaymentPoll(orderId);return {error:safeError(error)};}
    }
    ...
    function cancelDraft(){stopPaymentPolling();replace({screen:'overview',question:'',safety:null,termsAccepted:false,paymentStatus:null,paymentOrderId:null,paymentInstructions:null,statusMessage:''});}
  ...
  function createController({doc,session}){
    ...
    function render(state){
      ...
      el.panel.hidden=!state.visible;el.patient.textContent=state.patientName?`Care Profile ที่เลือก: ${state.patientName}`:'';el.entry.hidden=state.screen!=='overview';el.refresh.hidden=!state.visible;el.flow.hidden=state.screen==='overview';el.question.hidden=state.screen!=='question';el.check.hidden=state.screen!=='question';el.cancel.hidden=state.screen==='overview';el.terms.hidden=state.screen!=='terms';el.termsVersion.textContent=state.eligibility?.termsVersion?`เงื่อนไขฉบับ ${state.eligibility.termsVersion}`:'';el.payment.hidden=state.screen!=='payment';el.continueButton.disabled=!state.termsAccepted||!state.eligibility?.termsVersion||state.loading;el.termsCheck.checked=state.termsAccepted;el.live.textContent=state.statusMessage;if(el.paymentText)el.paymentText.textContent=state.paymentStatus==='queued'?'ชำระเงินแล้ว • รอเภสัชกรรับเคส':state.paymentStatus==='payment_pending'?'สแกน QR ด้วยแอปธนาคารเพื่อชำระ 100 บาท':'ระบบชำระเงินยังไม่พร้อมใช้งาน';if(el.paymentQr){const qr=state.paymentInstructions?.qrImageUrl;el.paymentQr.hidden=!(typeof qr==='string'&&qr.startsWith('https://'));if(!el.paymentQr.hidden)el.paymentQr.src=qr;else el.paymentQr.removeAttribute('src');}renderCaseGroups(doc,el.groups,state.collections,(id)=>session.selectCase(id));
