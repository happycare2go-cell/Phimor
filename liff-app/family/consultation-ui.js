(function initFamilyConsultationUI(root,factory){
  ...
  function createFamilyConsultationSession({request,onChange=()=>{},schedule=setTimeout,cancelSchedule=clearTimeout,pollSeconds=5,cryptoApi=globalThis.crypto,now=()=>new Date(),checkoutAdapter=null,paymentStatusAdapter=null}={}){
    ...
    return {snapshot,setProfile,checkEligibility,refreshCases,openQuestion,setQuestion,checkQuestion,acceptTerms,continueToPayment,pollPaymentStatus,cancelDraft,selectCase,pollOnce,sendMessage,setDocumentVisible,unmount};
  }
  ...
  function createController({doc,session}){
    ...
    function render(state){
      ...
      el.panel.hidden=!state.visible;el.patient.textContent=state.patientName?`Care Profile ที่เลือก: ${state.patientName}`:'';el.entry.hidden=state.screen!=='overview';el.refresh.hidden=!state.visible;el.flow.hidden=state.screen==='overview';el.question.hidden=state.screen!=='question';el.check.hidden=state.screen!=='question';el.cancel.hidden=state.screen==='overview';el.terms.hidden=state.screen!=='terms';el.termsVersion.textContent=state.eligibility?.termsVersion?`เงื่อนไขฉบับ ${state.eligibility.termsVersion}`:'';el.payment.hidden=state.screen!=='payment';el.continueButton.disabled=!state.termsAccepted||!state.eligibility?.termsVersion||state.loading;el.termsCheck.checked=state.termsAccepted;el.live.textContent=state.statusMessage;if(el.paymentText)el.paymentText.textContent=paymentStatusLabel(state.paymentStatus);if(el.paymentQr){const qr=safePaymentQrUrl(state.paymentInstructions?.qrImageUrl);el.paymentQr.hidden=!qr;if(qr)el.paymentQr.src=qr;else el.paymentQr.removeAttribute('src');}renderCaseGroups(doc,el.groups,state.collections,(id)=>session.selectCase(id));
      const selected=state.selectedCase;el.chat.hidden=!selected;el.chat.className=`consultation-chat${selected?` consultation-chat--${effectiveClosed(selected)?'closed':selected.state}`:''}`;if(selected){clearNode(el.header);textElement(doc,el.header,'h4','',`Consult Case ${safeText(selected.caseId).slice(-8)}`);if(selected.state==='queued')textElement(doc,el.header,'p','consultation-help','ชำระเงินแล้ว • รอเภสัชกรรับเคส ระยะเวลา 24 ชั่วโมงจะเริ่มเมื่อเภสัชกรรับเคส');else if(selected.state==='resolved')textElement(doc,el.header,'p','consultation-help','เภสัชกรตอบประเด็นหลักแล้ว คุณยังสามารถถามต่อเกี่ยวกับเคสนี้ได้จนถึงเวลาสิ้นสุด');else if(effectiveClosed(selected))textElement(doc,el.header,'p','consultation-help','หมดเวลาปรึกษาแล้ว ครบระยะเวลา 24 ชั่วโมงของ Consult Case นี้ ข้อความเดิมยังอ่านย้อนหลังได้');else textElement(doc,el.header,'p','consultation-help',`เภสัชกรรับเคส ${new Date(selected.acceptedAt).toLocaleString('th-TH')} • ห้องปรึกษานี้เปิดถึง ${new Date(selected.expiresAt).toLocaleString('th-TH')} • ${remainingLabel(selected)}`);renderMessages(doc,el.messages,state.messages);el.composer.disabled=state.sending||!canMessage(selected);el.send.disabled=state.sending||!canMessage(selected)||!el.composer.value.trim();}
    }
    el.entry.addEventListener('click',()=>session.openQuestion());el.refresh.addEventListener('click',()=>session.refreshCases());el.question.addEventListener('input',()=>session.setQuestion(el.question.value));el.check.addEventListener('click',()=>session.checkQuestion());el.cancel.addEventListener('click',()=>{session.cancelDraft();el.question.value='';});el.termsCheck.addEventListener('change',()=>session.acceptTerms(el.termsCheck.checked));el.continueButton.addEventListener('click',()=>session.continueToPayment());el.composer.addEventListener('input',()=>render(session.snapshot()));el.send.addEventListener('click',async()=>{const value=el.composer.value;const result=await session.sendMessage(value);if(!result?.ignored&&!result?.error)el.composer.value='';render(session.snapshot());});doc.addEventListener?.('visibilitychange',()=>session.setDocumentVisible(doc.visibilityState!=='hidden'));render(session.snapshot());return {render};
  }
  return Object.freeze({COLLECTIONS,COLLECTION_LABELS,normalizeMessages,mergeMessages,effectiveClosed,canMessage,remainingLabel,categorizeCases,paymentStatusLabel,safePaymentQrUrl,createFamilyConsultationSession,renderMessages,renderCaseGroups,createController});
}));
