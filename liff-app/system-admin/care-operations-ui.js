(function initCareOperationsUI(root, factory) {
  ...
  const buildIntegrationMappingInspectorRequest=(integrationClientId)=>({
    path:`/api/admin/platform/integration-clients/${encodeURIComponent(integrationClientId)}/control/mapping`,options:{method:'GET'},
  });
  const buildIntegrationIdentityInspectorRequest=(integrationClientId,{status='',page=1,limit=20}={})=>({
    path:`/api/admin/platform/integration-clients/${encodeURIComponent(integrationClientId)}/control/identities?${new URLSearchParams({status,page:String(page),limit:String(limit)}).toString()}`,options:{method:'GET'},
  });
  const buildIntegrationHistoryRequest=({integrationClientId='',status='',category='',from='',to='',reference='',page=1,limit=20}={})=>({
    path:`/api/admin/platform/integration-control/history?${new URLSearchParams({integrationClientId,status,category,from,to,reference,page:String(page),limit:String(limit)}).toString()}`,options:{method:'GET'},
  });
  const buildIntegrationHistoryDetailRequest=(eventKey)=>({path:`/api/admin/platform/integration-control/history/${encodeURIComponent(eventKey)}`,options:{method:'GET'}});
  ...
  function createController({ doc, request, confirmAction = async () => true }) {
    ...
    async function openMappingInspector(client){
      const dialog=ensureDialog('integrationMappingInspectorDialog','ดูการจับคู่ข้อมูล');const {live,body}=dialogParts(dialog);live.textContent='กำลังโหลดการจับคู่ข้อมูล…';live.className='care-ops__dialog-live';body.replaceChildren();openDialog(dialog);
      try{const result=await send(buildIntegrationMappingInspectorRequest(client.integrationClientId));live.textContent='';
        const intro=element('section','care-ops__detail-section');intro.append(element('h3','',result.integrationClient?.displayName||client.displayName||'ระบบเชื่อมต่อ'),element('p','care-ops__meta',result.message||'ไม่มีข้อมูล'));
        if(result.activeAdapter)intro.append(element('div','care-ops__mapping-path',`${result.activeAdapter.sourceSystem||'ระบบต้นทาง'} → ${result.activeAdapter.displayName} V${result.activeAdapter.version||'?'} → PHIMOR`));
        const list=element('div','care-ops__mapping-inspector');
        safeArray(result.mappings).forEach((item)=>{const row=element('article','care-ops__mapping-row');row.append(element('strong','',item.phimorLabel||'ข้อมูลมาตรฐาน'),element('div','care-ops__meta',`${item.section||'ข้อมูลมาตรฐาน'} · ${item.required?'จำเป็น':'ไม่บังคับ'} · ${item.transformation||'ไม่ทราบวิธีแปลง'}`),element('code','care-ops__source-path',safeArray(item.sourcePaths).join(' + ')||'ไม่ทราบ source field'),element('div','care-ops__mapping-arrow','↓'),element('code','care-ops__canonical-field',item.canonicalField||'ไม่ทราบข้อมูลมาตรฐาน'));list.append(row)});
        if(!safeArray(result.mappings).length)list.append(element('div','care-ops__empty','ยังไม่มี field mapping สำหรับตรวจสอบ ระบบนี้รับข้อมูลตาม canonical contract'));
        body.replaceChildren(intro,list,element('p','care-ops__meta','หน้าจอนี้เป็นแบบอ่านอย่างเดียวและไม่แสดงค่าตัวอย่างจาก event'));
      }catch(error){showDialogError(dialog,errorMessage(error));body.replaceChildren(element('div','care-ops__error','โหลดการจับคู่ข้อมูลไม่สำเร็จ'));}
    }
    async function openIdentityInspector(client,page=1){
      const dialog=ensureDialog('integrationIdentityInspectorDialog','ดูการเชื่อมตัวตน');const {live,body}=dialogParts(dialog);live.textContent='กำลังโหลดการเชื่อมตัวตน…';live.className='care-ops__dialog-live';if(page===1)body.replaceChildren();openDialog(dialog);
      try{const result=await send(buildIntegrationIdentityInspectorRequest(client.integrationClientId,{page,limit:20}));live.textContent='';const intro=element('section','care-ops__detail-section');intro.append(element('h3','',result.integrationClient?.displayName||client.displayName||'ระบบเชื่อมต่อ'),element('p','care-ops__meta','แสดงการเชื่อมตัวตนปัจจุบันจาก mapping และความสัมพันธ์ที่ backend ตรวจสอบได้'));
        const list=element('div','care-ops__identity-list');safeArray(result.items).forEach((item)=>{const card=element('article','care-ops__identity-chain');const method={learned:'เรียนรู้อัตโนมัติ',configured:'ผู้ดูแลกำหนด',exact:'ตรงกันแบบ exact',manual:'กำหนดด้วยผู้ดูแล',unresolved:'ยังไม่เชื่อม',unknown:'ไม่ทราบวิธีเชื่อม'}[item.matchMethod]||'ไม่ทราบวิธีเชื่อม';card.append(element('strong','',`${item.externalCenterId||'ไม่ทราบรหัสศูนย์'} · ${item.externalResidentId||'ไม่ทราบรหัสผู้พัก'}`),element('div','care-ops__meta',`${item.safeReference||'ไม่มีรหัสอ้างอิง'} · ${method} · ${formatDate(item.lastSeenAt)}`));const chain=element('ol','care-ops__identity-steps');const add=(label,value,state)=>{const row=element('li',`care-ops__identity-step care-ops__identity-step--${state}`);row.append(element('span','',label),element('strong','',value));chain.append(row)};add('ระบบภายนอก',item.externalResidentId||'ไม่มีข้อมูล',item.mappingStatus==='ambiguous'?'attention':'completed');add('PHIMOR Center',item.center?.displayName||({inactive:'ไม่ได้ใช้งาน',unresolved:'ยังไม่ได้เชื่อม'}[item.center?.state]||'ไม่มีข้อมูล'),item.center?.state);add('PHIMOR Resident',item.resident?.displayName?`${item.resident.displayName}${item.resident.room?` · ห้อง ${item.resident.room}`:''}`:item.mappingStatus==='ambiguous'?`พบ ${item.ambiguity?.candidateCount||0} รายการที่อาจตรงกัน`:'ยังไม่ได้เชื่อม',item.mappingStatus==='ambiguous'?'attention':item.resident?.state);add('Care Profile',item.careProfile?.state==='resolved'?'พร้อมใช้งาน':item.careProfile?.state==='missing'?'ไม่พบ Care Profile':'ยังตรวจสอบไม่ได้',item.careProfile?.state);add('ปลายทางครอบครัว',item.familyDestination?.state==='resolved'?'เชื่อมแล้ว':item.familyDestination?.state==='missing'?'ยังไม่ได้ผูกปลายทางครอบครัว':'ยังตรวจสอบไม่ได้',item.familyDestination?.state);card.append(chain);if(['pending_subject_mapping','ambiguous'].includes(item.mappingStatus)){const actions=element('div','care-ops__actions');actions.append(button('ไปที่ผู้พักรอเชื่อม',()=>{closeDialog(dialog);setTab(item.mappingStatus==='ambiguous'?'alerts':'pending');load({tabs:[item.mappingStatus==='ambiguous'?'alerts':'pending']});},'secondary'));card.append(actions)}list.append(card)});if(!safeArray(result.items).length)list.append(element('div','care-ops__empty','ยังไม่มีข้อมูลการเชื่อมตัวตน'));
        const controls=pager(result.pagination,(next)=>openIdentityInspector(client,next));body.replaceChildren(intro,list,controls,element('p','care-ops__meta','ไม่แสดง LINE ID ข้อมูลสุขภาพ หรือปลายทางกลุ่มแบบเต็ม'));
      }catch(error){showDialogError(dialog,errorMessage(error));body.replaceChildren(element('div','care-ops__error','โหลดการเชื่อมตัวตนไม่สำเร็จ'));}
    }
    function openWorkQueueFromHistory(dialog){closeDialog(dialog);const destination=doc.querySelector('[data-shell-destination="review"]');if(destination)destination.click();}
    async function openHistoryDetail(eventKey){
      const dialog=ensureDialog('integrationHistoryDetailDialog','รายละเอียดเหตุการณ์');const {live,body}=dialogParts(dialog);live.textContent='กำลังโหลดรายละเอียด…';live.className='care-ops__dialog-live';body.replaceChildren();openDialog(dialog);
      try{const result=await send(buildIntegrationHistoryDetailRequest(eventKey));const item=result.item||{};live.textContent='';const origin=element('section','care-ops__detail-section');origin.append(element('h3','','1. แหล่งข้อมูล'),element('p','',item.integration?.displayName||'ไม่ทราบระบบ'),element('p','care-ops__meta',`${item.integration?.sourceSystem||'ไม่ทราบ Source system'} · ${item.safeReference||'ไม่มีรหัสอ้างอิง'}`));
        const time=element('section','care-ops__detail-section');time.append(element('h3','','2. เวลาที่รับ'),element('p','',formatDate(item.receivedAt)));
        const event=element('section','care-ops__detail-section');event.append(element('h3','','3. ประเภทเหตุการณ์'),element('p','',item.eventType||'ไม่มีข้อมูล'),element('p','care-ops__meta',item.outcomeLabel||'ไม่ทราบผล'));
        const stages=element('section','care-ops__detail-section');stages.append(element('h3','','4–5. เส้นทางและจุดที่ติด'),renderFlowStages(item.detail));
        const identity=element('section','care-ops__detail-section');identity.append(element('h3','','6–7. ตัวตนและ Care Profile'),element('p','',`ศูนย์: ${item.identity?.centerName||'ไม่มีข้อมูล'}`),element('p','',`ผู้พัก: ${item.identity?.residentName||'ยังตรวจสอบไม่ได้'}${item.identity?.room?` · ห้อง ${item.identity.room}`:''}`),element('p','',`Care Profile: ${item.identity?.careProfileState==='resolved'?'เชื่อมแล้ว':'ยังตรวจสอบไม่ได้'}`));
        const destination=element('section','care-ops__detail-section');destination.append(element('h3','','8. ปลายทางครอบครัว'),element('p','',({resolved:'พบปลายทางที่ผ่านการตรวจสอบแล้ว',attention:'ต้องตรวจสอบ GroupBinding',not_applicable:'ไม่เกี่ยวข้องกับเหตุการณ์นี้',unknown:'ยังตรวจสอบไม่ได้'}[item.familyDestination?.state]||'ยังตรวจสอบไม่ได้')));
        const notification=element('section','care-ops__detail-section');notification.append(element('h3','','9. สถานะการแจ้งเตือน'),element('p','',item.notification?.providerStateLabel||'ไม่มีข้อมูล'),element('p','care-ops__meta',item.notification?`ระบบดำเนินการ ${item.notification.attempts||0} ครั้ง${item.notification.nextAttemptAt?` · จะลองอีกครั้ง ${formatDate(item.notification.nextAttemptAt)}`:''}`:'ระบบไม่มีหลักฐานสถานะ provider สำหรับเหตุการณ์นี้'));if(item.notification?.status==='sent')notification.append(element('p','care-ops__notice','สถานะนี้หมายถึงผู้ให้บริการรับคำขอส่ง ไม่ใช่หลักฐานว่าผู้รับอ่านข้อความแล้ว'));
        const next=element('section','care-ops__detail-section');next.append(element('h3','','10. ควรตรวจอะไรต่อ'),element('p','',item.nextOperatorAction||'ยังตรวจสอบไม่ได้'));if(['retrying','dead_letter'].includes(item.notification?.status)||['attention','failed'].some((value)=>safeArray(item.detail?.stages).some((stage)=>stage.state===value))){next.append(button('เปิดงานต้องตรวจ',()=>openWorkQueueFromHistory(dialog),'secondary'));}
        const technical=element('details','care-ops__technical');technical.append(element('summary','','รายละเอียดทางเทคนิค'),element('div','care-ops__meta',`Event: ${item.detail?.technical?.safeEventReference||'ไม่มีข้อมูล'}`),element('div','care-ops__meta',`Integration: ${item.detail?.technical?.safeIntegrationReference||'ไม่มีข้อมูล'}`),element('div','care-ops__meta',`Processing: ${item.detail?.technical?.processingState||'unknown'} · ${item.detail?.technical?.processingAttemptCount||0} ครั้ง`),element('div','care-ops__meta',`Adapter version: ${item.detail?.technical?.adapterVersion||'ไม่มีหลักฐานระดับ event'}`),element('div','care-ops__meta',`Error code: ${item.detail?.technical?.safeErrorCode||'ไม่มีข้อมูล'}`));body.replaceChildren(origin,time,event,stages,identity,destination,notification,next,technical);
      }catch(error){showDialogError(dialog,errorMessage(error));body.replaceChildren(element('div','care-ops__error','โหลดรายละเอียดเหตุการณ์ไม่สำเร็จ'));}
    }
    async function openIntegrationHistory({integrationClientId='',page=1,filters=null}={}){
      const dialog=ensureDialog('integrationHistoryDialog','ประวัติเหตุการณ์');const {live,body}=dialogParts(dialog);const current=filters||dialog.__historyFilters||{status:'',category:'',from:'',to:'',reference:''};dialog.__historyFilters=current;live.textContent='กำลังโหลดประวัติ…';live.className='care-ops__dialog-live';if(page===1)body.replaceChildren();openDialog(dialog);
      try{const result=await send(buildIntegrationHistoryRequest({integrationClientId,...current,page,limit:20}));live.textContent='';const tools=element('div','care-ops__history-tools');const status=selectField('ผลการประมวลผล',[{value:'',label:'ทุกสถานะ'},{value:'processed',label:'บันทึกข้อมูลแล้ว'},{value:'pending',label:'รอดำเนินการ'},{value:'rejected',label:'ถูกปฏิเสธ'},{value:'retrying',label:'กำลังลองใหม่'},{value:'dead',label:'หยุดประมวลผล'}],current.status);const category=selectField('ขั้นตอนที่ต้องตรวจ',[{value:'',label:'ทุกขั้นตอน'},{value:'identity',label:'การจับคู่ผู้พัก'},{value:'family_destination',label:'ปลายทางครอบครัว'},{value:'notification',label:'การส่งแจ้งเตือน'},{value:'processing',label:'การประมวลผล'}],current.category);const from=input('ตั้งแต่วันที่',{type:'date',value:current.from});const to=input('ถึงวันที่',{type:'date',value:current.to});const reference=input('รหัสอ้างอิงแบบตรงกัน',{maxLength:80,value:current.reference,placeholder:'เช่น 00AB12'});const search=button('ค้นหา',()=>{Object.assign(current,{status:status.control.value,category:category.control.value,from:from.control.value,to:to.control.value,reference:reference.control.value.trim()});openIntegrationHistory({integrationClientId,page:1,filters:current});},'secondary');tools.append(status.label,category.label,from.label,to.label,reference.label,search);
        const list=element('div','care-ops__history-list');safeArray(result.items).forEach((item)=>{const card=itemCard(item.integration?.displayName||'ระบบเชื่อมต่อ',`${item.safeReference||'ไม่มีรหัสอ้างอิง'} · รับเมื่อ ${formatDate(item.receivedAt)}`);card.append(element('strong','',item.outcomeLabel||'ไม่ทราบผล'),element('p','care-ops__meta',item.summary||'ยังตรวจสอบไม่ได้'));const actions=element('div','care-ops__actions');actions.append(button('ดูรายละเอียด',()=>openHistoryDetail(item.eventKey),'secondary'));card.append(actions);list.append(card)});if(!safeArray(result.items).length)list.append(element('div','care-ops__empty','ไม่พบประวัติเหตุการณ์ที่ตรงกับเงื่อนไข'));
        body.replaceChildren(tools,list,pager(result.pagination,(next)=>openIntegrationHistory({integrationClientId,page:next,filters:current})),element('p','care-ops__meta','ประวัตินี้ไม่แสดง payload ข้อมูลสุขภาพหรือปลายทาง LINE และไม่มีคำสั่งส่งซ้ำ'));
      }catch(error){showDialogError(dialog,errorMessage(error));body.replaceChildren(element('div','care-ops__error','โหลดประวัติเหตุการณ์ไม่สำเร็จ'));}
    }
    ...
    function renderIntegrations() {
      ...
      header.append(search.label);if(statusFilter)header.append(statusFilter.label);if(!archived)header.append(button('ประวัติเหตุการณ์',()=>openIntegrationHistory({}), 'secondary'),button('+ เพิ่มระบบเชื่อมต่อ',openCreateClient,''));
      const list=element('div','care-ops__list');
  ...
  return { CAPABILITY_LABELS, SUPPORTED_EVENT_TYPES, CLIENT_STATUS_LABELS, GROUP_LABELS, IDENTITY_POLICY_LABELS,
    ...
    buildClientStatusRequest, buildIntegrationDirectoryRequest, buildIntegrationOverviewRequest, buildIntegrationMappingInspectorRequest, buildIntegrationIdentityInspectorRequest, buildIntegrationHistoryRequest, buildIntegrationHistoryDetailRequest, buildCenterScopeRequest, buildEventScopeRequest, buildCredentialRequest,
