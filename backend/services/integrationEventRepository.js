...
function createIntegrationEventRepository({queryFn=databaseQuery}={}){const one=async(sql,p=[])=>(await queryFn(sql,p)).rows[0]||null;const many=async(sql,p=[])=>(await queryFn(sql,p)).rows;
  return{
    insertEvent(r){return one(`INSERT INTO integration_event_inbox (
      ...
      expected_line_group_id
    ) VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,'received',$8,$9,$10,$11)
    ...
    markProcessed(id,{residentId,careProfileId,resourceType,resourceId,expectedLineGroupId=null,
      verifiedLineGroupId=null,groupReconciliationStatus=null,notificationIntentStatus='not_applicable'}){return one(`UPDATE integration_event_inbox SET status='processed',resident_id=$2,care_profile_id=$3,
      canonical_resource_type=$4,canonical_resource_id=$5,pending_reason=NULL,last_error_code=NULL,next_attempt_at=NULL,
      expected_line_group_id=$6,verified_line_group_id=$7,group_reconciliation_status=$8,
      notification_intent_status=$9,processed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
      WHERE integration_event_id=$1 RETURNING *`,[id,residentId,careProfileId,resourceType,resourceId,
      expectedLineGroupId,verifiedLineGroupId,groupReconciliationStatus,notificationIntentStatus]);},
    updateRouting(id,{expectedLineGroupId=null,verifiedLineGroupId=null,groupReconciliationStatus,
      notificationIntentStatus}){return one(`UPDATE integration_event_inbox SET expected_line_group_id=$2,
      verified_line_group_id=$3,group_reconciliation_status=$4,notification_intent_status=$5,
      updated_at=CURRENT_TIMESTAMP WHERE integration_event_id=$1 AND status='processed' RETURNING *`,
    [id,expectedLineGroupId,verifiedLineGroupId,groupReconciliationStatus,notificationIntentStatus]);},
    ...
    listOperational({integrationClientId=null,organizationId=null,centerId=null,
      groupStatus=null,limit=100}={}){const p=[];const where=[];
      if(integrationClientId){p.push(integrationClientId);where.push(`integration_client_id=$${p.length}`);}
      if(organizationId){p.push(organizationId);where.push(`organization_id=$${p.length}`);}
      if(centerId){p.push(centerId);where.push(`center_id=$${p.length}`);}
      if(groupStatus){p.push(groupStatus);where.push(`group_reconciliation_status=$${p.length}`);}
      p.push(limit);return many(`SELECT integration_event_id,integration_client_id,organization_id,
        external_event_id,event_type,status,external_center_id,external_resident_id,center_id,
        resident_id,canonical_resource_type,canonical_resource_id,pending_reason,last_error_code,
        expected_line_group_id,verified_line_group_id,group_reconciliation_status,
