...
const RULES = Object.freeze([
  ...
  { intent: 'compare', patterns: [/(ยา|รายการยา).*(เปลี่ยน|ต่าง|แตกต่าง).*อะไร/i, /(เปรียบเทียบ|เทียบ).*(ยา|snapshot|รายการ)/i, /รอบล่าสุด.*ยา.*เปลี่ยน/i] },
  { intent: 'summarize', patterns: [/(ช่วย)?\s*สรุป.*(ข้อมูลสุขภาพ|care profile|ประวัติ)/i, /ภาพรวม.*สุขภาพ/i] },
  { intent: 'retrieve', patterns: [/(ตอนนี้|ปัจจุบัน).*(มี|ใช้|กิน).*ยา.*(อะไร|บ้าง)/i, /ยา.*ที่บันทึก.*(กิน|ใช้).*(ตอนไหน|อย่างไร|ยังไง)/i, /(ดู|ขอ|บอก).*(รายการยา|ยาปัจจุบัน|วิธีใช้ที่บันทึก)/i] },
