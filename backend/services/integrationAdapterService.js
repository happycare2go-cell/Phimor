...
function createIntegrationAdapterService(overrides={}){
  ...
  async function startCapture({integrationClientId,targetEventType=TARGET_EVENT_TYPE,actorReference}){
    const client=await requireClient(integrationClientId);assertTarget(client,targetEventType);const sourceSystemKey=normalizeSourceSystemKey(client.sourceSystem);
  ...
  async function reusableForSample(client,sample){const candidates=await repository.listReusableTemplates(normalizeSourceSystemKey(client.sourceSystem),sample.target_event_type);const reusable=[];
    for(const row of candidates){const analysis=compareStructure({baselineStructure:row.source_structure,mappingRules:row.mapping_rules,payload:sample.sample_payload});if(analysis.breaking.length)continue;try{transformPayload({integrationClientId:client.integrationClientId,targetEventType:sample.target_event_type,rules:row.mapping_rules,payload:sample.sample_payload});}catch(_){continue;}reusable.push({...versionProjection(row),exactFingerprint:analysis.exact});}
    return reusable;
  }
  ...
  async function createDraft({integrationClientId,sampleId,mappingRules,actorReference}){
    const client=await requireClient(integrationClientId);const sample=await requireSample(client,sampleId);const fields=discoverFields(sample.sample_payload);const rules=validateMappingRules(mappingRules,fields);
  ...
  async function activateAdapter({integrationClientId,sampleId,adapterProfileId,actorReference}){
    await previewAdapter({integrationClientId,sampleId,adapterProfileId});const client=await requireClient(integrationClientId);const draft=await repository.findVersion(adapterProfileId);assertTemplateSource(client,draft);
  ...
  async function reuseAdapter({integrationClientId,sampleId,adapterVersionId,actorReference}){
    const client=await requireClient(integrationClientId);const sample=await requireSample(client,sampleId);const version=await repository.findVersion(adapterVersionId);if(!version||version.status!=='active')throw new IntegrationAdapterError('ADAPTER_REUSE_NOT_AVAILABLE','ไม่พบรูปแบบข้อมูลที่พร้อมใช้',404);assertTemplateSource(client,version);
    const analysis=compareStructure({baselineStructure:version.source_structure,mappingRules:version.mapping_rules,payload:sample.sample_payload});if(analysis.breaking.length)throw new IntegrationAdapterError('ADAPTER_SOURCE_CHANGED','รูปแบบข้อมูลตัวอย่างไม่ตรงกับ Adapter',422);transformPayload({integrationClientId,targetEventType:sample.target_event_type,rules:version.mapping_rules,payload:sample.sample_payload});
  ...
  async function prepareInbound({identity,input}){
    ...
    let canonical;try{canonical=transformPayload({integrationClientId:identity.integrationClientId,targetEventType:binding.target_event_type,rules:binding.mapping_rules,payload:sanitized.payload});}catch(error){if(String(error?.code||'').startsWith('ADAPTER_'))await recordNotice(binding,error.code==='ADAPTER_SOURCE_CHANGED'?'ADAPTER_SOURCE_CHANGED':'ADAPTER_TRANSFORM_FAILURE',{sourcePath:'mapped_source',fieldKey:crypto.createHash('sha256').update(String(error.code)).digest('hex')});throw error;}
