...
test('Center role labels and controls are reapplied on every authoritative Center switch', () => {
  ...
  assert.match(centerHtml, /addResidentCard'\)\.style\.display=canManage/);
  assert.match(centerHtml, /querySelector\('\.tab\[data-view="transport"\]'\)\.style\.display=canManage/);
  assert.match(centerHtml, /querySelector\('\.tab\[data-view="ratecard"\]'\)\.style\.display=canManage/);
  assert.match(centerHtml, /staffTab'\)\.style\.display=role==='owner'/);
  ...
  assert.match(selectSource, /CENTERS\.find\(c => c\.center_id === centerId\)/);
  assert.match(selectSource, /applySelectedCenterContext\(center\)/);
